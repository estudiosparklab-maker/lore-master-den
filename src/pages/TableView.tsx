...
import { ArrowLeft, Plus, Swords, Copy } from 'lucide-react';
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6 flex items-start justify-between">
          ...
          <div className="flex gap-2">
            ...
            {!isMaster && !characters.some(c => c.user_id === user?.id) && (
              <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 rounded-sm border border-gold bg-gold/10 px-3 py-2 font-cinzel text-xs text-gold hover:bg-gold/20 transition-colors">
                <Plus className="h-3 w-3" /> Criar Minha Ficha
              </button>
            )}
          </div>
        </div>
        ...
        {/* Current Turn Banner */}
        {currentTurnChar && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-medieval mb-6 flex items-center justify-between p-4 glow-gold">
